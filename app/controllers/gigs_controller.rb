class GigsController < ApplicationController
  ...
  def is_authorised
    redirect_to root_path, alert: "You do not have the permission" unless current_user.id = @gig.user_id
  end

  def gig_params
    params.require(:gig).permit(:title, :description, :video, :active, :category_id, :has_single_pricing, pricings_attributes:[:id, :title, :description, :delivery_time, :price, :pricing_type])
  end
end
